...
char aw_rtp_name[][AW_RTP_NAME_MAX] = {
	//vendor
	{"haptic_rtp_osc_24K_5s.bin"},//double = 91
	{"haptic_rtp.bin"},
...
static int ctrl_init(struct aw_haptic *aw_haptic)
{
	...
	for (cnt = 0; cnt < AW_READ_CHIPID_RETRIES; cnt++) {
		...
		if (read_chipid(aw_haptic, &reg) < 0)
			aw_err("read chip id fail reg=%d.",reg);
		switch (reg) {
...
static void ram_play(struct aw_haptic *aw_haptic, uint8_t mode)
{
	...
	if (aw_haptic->dual_flag) {
		...
		if (down_trylock(&left->sema) == 0) {
			if (down_interruptible(&left->sema))
				aw_err("down_interruptible error");
		} else {
			up(&left->sema);
			up(&left->sema);
		}
...
static void rtp_work_routine(struct work_struct *work)
{
	...
	pr_info("aw_haptic->state = %d", aw_haptic->state);
	if (!aw_haptic->state) {
		mutex_unlock(&aw_haptic->lock);
		return;
	}
				   ...
				   const char *buf, size_t count)
{
	...
	struct aw_haptic *aw_haptic = container_of(cdev, struct aw_haptic,
						   vib_dev);
	scanf_num = sscanf(buf, "%d %d", &duration_l, &duration_r);
	if (scanf_num == 2) {
		mutex_lock(&aw_haptic->lock);
...
//[TASK][PIKE-2976][vibrator] Add gain value configuration for King glory scene by luoming0812 at 20220309
			...
			left->rtp_file_num = effect_id+1-RTP_L_VIB_BASE;
			...
			right->rtp_file_num = effect_id+1-RTP_R_VIB_BASE;
			...
			left->rtp_file_num = effect_id +1-RTP_D_VIB_BASE;
			right->rtp_file_num =  effect_id +1-RTP_D_VIB_BASE;
		}
		mutex_unlock(&aw_haptic->lock);
	}
				   ...
				   const char *buf, size_t count)
{
	...
	mutex_lock(&aw_haptic->lock);
	if (down_trylock(&left->sema) == 0)
		aw_info("down_trylock(&left->sema) == 0");
	up(&left->sema);
	left->state = val;
	right->state = val;
	left->dual_flag = true;
...
static DEVICE_ATTR(double, S_IWUSR | S_IRUGO, double_show, double_store);
