...
/* add for pen delay end  */

uint8_t panel_current_fps = 0;
#if NVT_TOUCH_ESD_PROTECT
...
{
	if (ts->mmap->ACI_ERR_CLR == 0)
		return;
	NVT_LOG("nvt_clear_aci_error_flag start");	
	nvt_write_addr(ts->mmap->ACI_ERR_CLR, 0xA5);
	nvt_write_addr(ts->mmap->ACI_ERR_CLR, 0x00);
...
int32_t nvt_check_stylus_state(uint8_t input_id, uint8_t *data)
{
	...
	//NVT_LOG("nvt_check_stylus_state input_id=%d func_type=%d panel_current_fps=%d.\n",input_id,func_type,panel_current_fps);
	if ((input_id == DATA_PROTOCOL) && (func_type == FUNCPAGE_STYLUS)) {
		//ret = stylus_state;
		if (stylus_state == DETECT_PEN) {
			//NVT_LOG("Detect pen.\n");
                        input_report_key(ts->input_dev, TOOL_TYPE_STYLUS, 1);
                        input_sync(ts->input_dev);
                        input_report_key(ts->input_dev, TOOL_TYPE_STYLUS, 0);
                        input_sync(ts->input_dev);
			ret = 1;
...
static int nvt_ts_check_dt(struct device_node *np)
{
	...
	//NVT_LOG("nvt panel count = %d\n",count);
	if (count <= 0){
		NVT_LOG("nvt of_count_phandle_with_args count <0\n");
		return 0;
	}
	//NVT_LOG("nvt panel count > 0\n");
	for (i = 0; i < count; i++) {
		node = of_parse_phandle(np, "panel", i);
		//NVT_LOG("node = %p\n",node);
		panel = of_drm_find_panel(node);
		of_node_put(node);
		 //NVT_LOG("IS_ERR(panel) = %p\n",IS_ERR(panel));
		 //NVT_LOG("panel = %p\n",panel);
		if (!IS_ERR(panel)) {
			active_panel = panel;
			return 0;
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	NVT_LOG("start nvt prepare for spi parameter\n");
	...
	if (ret) {
	 ...
	 NVT_LOG("request and config GPIOs ok\n");
	mutex_init(&ts->lock);
...
#endif
	input_set_capability(ts->input_dev, EV_KEY, TOOL_TYPE_STYLUS);
...
#if IS_ENABLED(CONFIG_QCOM_PANEL_EVENT_NOTIFIER)
	if (active_panel) {
		NVT_LOG("yh test panel_event_notifier_register\n");
		cookie = panel_event_notifier_register(PANEL_EVENT_NOTIFICATION_PRIMARY,
				PANEL_EVENT_NOTIFIER_CLIENT_PRIMARY_TOUCH, active_panel,
				&nvt_panel_notifier_callback, ts);
	}else {
		NVT_LOG("yh test no panel_event_notifier_register\n");
	}
...
*/
	switch (notification->notif_type) {
		...
		case DRM_PANEL_EVENT_FPS_CHANGE:
			panel_current_fps = notification->notif_data.new_fps;
			NVT_LOG("Received fps change old fps:%d new fps:%d cur_fps:%d\n",
			notification->notif_data.old_fps,
			notification->notif_data.new_fps, panel_current_fps);
...
//	msleep(4000);
	...
	NVT_LOG("nvt driver init finished\n");
