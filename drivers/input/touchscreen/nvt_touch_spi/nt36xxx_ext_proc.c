...
#define NVT_FW_REFLASH "nvt_fw_reflash"
#endif
...
static struct proc_dir_entry *NVT_proc_fw_reflash_entry;
#if NVT_CUST_PROC_CMD
...
/*
 *Description:
 *	Novatek touchscreen /proc/nvt_fw_reflash open
 *	function.
 *
 *	return:
 *	n.a.
...
static ssize_t nvt_fw_reflash_store(struct file *file, const char *buffer, size_t count, loff_t *pos) {
	char dbg[10] = { 0 };
	int res = 0;
	uint8_t value;
	res = copy_from_user(dbg, (uint8_t *) buffer, sizeof(uint8_t));
...
#ifdef HAVE_PROC_OPS
static const struct proc_ops nvt_fw_reflash_fops = {
	.proc_write = nvt_fw_reflash_store,
};
#else
static const struct file_operations nvt_fw_reflash_fops = {
	.owner = THIS_MODULE,
	.write = nvt_fw_reflash_store,
};
#endif
...
int32_t nvt_extra_proc_init(void)
{
	...
	NVT_proc_fw_reflash_entry = proc_create(NVT_FW_REFLASH, 0222, NULL, &nvt_fw_reflash_fops);
	if (NVT_proc_fw_reflash_entry == NULL) {
		NVT_ERR("create proc/%s Failed!\n", NVT_FW_REFLASH);
		return -ENOMEM;
	} else {
		NVT_LOG("create proc/%s Succeeded!\n", NVT_FW_REFLASH);
...
void nvt_extra_proc_deinit(void)
{
	...
	if (NVT_proc_fw_reflash_entry != NULL) {
		remove_proc_entry(NVT_FW_REFLASH, NULL);
		NVT_proc_fw_reflash_entry = NULL;
		NVT_LOG("Removed /proc/%s \n", NVT_FW_REFLASH);
	}
#if NVT_CUST_PROC_CMD
	if (NVT_proc_edge_reject_entry != NULL) {
