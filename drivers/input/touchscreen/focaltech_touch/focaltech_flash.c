...
#endif
    fts_irq_enable();
    ret = fts_write_reg(FTS_REG_CHARGER_MODE_EN, upg->ts_data->charger_mode);
    pr_info("fts: charger mode en = %d, ret = %d\n", upg->ts_data->charger_mode, ret);
    fts_probe_status = true;
    upg->ts_data->fw_loading = 0;
}
