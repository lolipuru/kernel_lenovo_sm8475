...
bool fts_probe_status = false;
bool fts_charger_mode = false;
/*****************************************************************************
* Static function prototypes
*****************************************************************************/
static int fts_ts_suspend(struct device *dev);
static int fts_ts_resume(struct device *dev);
static int fts_fp_match = 0;
int zui_finger_touch = 0;
int zui_atr_key_touch = 0;
int zui_atr_key_touch_0 = 0;
int zui_atr_key_touch_1 = 0;
int zui_atr_key_touch_2 = 0;
int zui_atr_key_touch_3 = 0;
int zui_num_of_fingers = 0;
#define ZUI_NUM_OF_SLOT 11
...
void zui_input_report_atr_key_base(int x, int y, int press, int slot_flag)
{
	struct fts_ts_data *ts_data = fts_data;
	struct input_dev *input_dev = ts_data->input_dev;
	int num_of_slot = 0;

        pr_info("zui_input_report_atr_key press %d, x %d, y %d, slot_flag %d\n", press, x, y, slot_flag);

        mutex_lock(&ts_data->report_mutex);
	switch (slot_flag) {
		case 0 :
			num_of_slot = zui_num_of_fingers - ZUI_NUM_OF_SLOT;
			break;
		case 1 :
			num_of_slot = zui_num_of_fingers - ZUI_NUM_OF_SLOT + 1;
			break;
		case 2 :
			num_of_slot = zui_num_of_fingers - ZUI_NUM_OF_SLOT + 2;
			break;
		case 3 :
		case 4 :
		case 5 :
		...
		case 8 :
		case 9 :
			num_of_slot = zui_num_of_fingers - ZUI_NUM_OF_SLOT + slot_flag;
			break;
		default :
			num_of_slot = zui_num_of_fingers - ZUI_NUM_OF_SLOT;
		}
	input_mt_slot(input_dev, num_of_slot);

        if (press) {
            input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, true);

#if 0 //FTS_REPORT_PRESSURE_EN
            input_report_abs(input_dev, ABS_MT_PRESSURE, 0x3f);
#endif
            input_report_abs(input_dev, ABS_MT_PRESSURE, 0xf);
            input_report_abs(input_dev, ABS_MT_TOUCH_MAJOR, 0x09);
            input_report_abs(input_dev, ABS_MT_TOUCH_MINOR, 0x09);
	...
	switch (slot_flag) {
		case 0 :
			if (!press)
				zui_atr_key_touch_0 = 0;
			else
				zui_atr_key_touch_0 = 1;
			break;
		case 1 :
			if (!press)
				zui_atr_key_touch_1 = 0;
			else
				zui_atr_key_touch_1 = 1;
			break;
		case 2 :
			if(!press)
				zui_atr_key_touch_2 = 0;
			else
				zui_atr_key_touch_2 = 1;
			break;
		case 3 :
			if(!press)
				zui_atr_key_touch_3 &= ~0x1;
			else
				zui_atr_key_touch_3  |= 0x1;
			break;
		case 4 :
			if(!press)
				zui_atr_key_touch_3 &= ~0x2;
			else
				zui_atr_key_touch_3  |= 0x2;
			break;
		case 5 :
			if(!press)
				zui_atr_key_touch_3 &= ~0x4;
			else
				zui_atr_key_touch_3  |= 0x4;
			break;
		case 6 :
			if(!press)
				zui_atr_key_touch_3 &= ~0x8;
			else
				zui_atr_key_touch_3  |= 0x8;
			break;
		case 7 :
			if(!press)
				zui_atr_key_touch_3 &= ~0x10;
			else
				zui_atr_key_touch_3  |= 0x10;
			break;
		case 8 :
			if(!press)
				zui_atr_key_touch_3 &= ~0x20;
			else
				zui_atr_key_touch_3  |= 0x20;
			break;
		case 9 :
			if(!press)
				zui_atr_key_touch_3 &= ~0x40;
			else
				zui_atr_key_touch_3  |= 0x40;
			break;
		default :
	 ...
	 pr_info("zui_input_report_atr_key touch_3 = 0x%x\n", zui_atr_key_touch_3);

	if ((zui_atr_key_touch_0 == 0) && (zui_atr_key_touch_1 == 0) && (zui_atr_key_touch_2 == 0) &&
		(zui_atr_key_touch_3 == 0))
		zui_atr_key_touch = 0;
	else
		zui_atr_key_touch = 1;

        if ((zui_atr_key_touch == 0) && (zui_finger_touch == 0))
...
{
	zui_input_report_atr_key_base(x, y, press, 0);
}

void zui_input_report_atr_key_1(int x, int y, int press)
{
        zui_input_report_atr_key_base(x, y, press, 1);
}
...
void zui_input_report_atr_key_n(int x, int y, int press, int num)
{
        zui_input_report_atr_key_base(x, y, press, num);
}

void fts_is_charger_enabled(int enable)
{
	int ret = 0;
	struct fts_ts_data *ts_data;

	fts_charger_mode = enable;
	if(fts_probe_status == false ||!fts_data || !fts_data->client ) {
		FTS_INFO("fts not read, en = %d\n", enable);
		return;
	}
	ts_data = fts_data;
	...
	if (ts_data->suspended) {
		FTS_INFO("suspend mode, chg en = %d\n", enable);
		return;
	}
	FTS_INFO("normal, chg en = %d\n", enable);
	if (enable) {
		ret = fts_write_reg(0x8b, 1);
		if (ret < 0)
			FTS_ERROR("set TP to enter charge mode, ret=%d", ret);
	} else {
		ret = fts_write_reg(0x8b, 0);
		if (ret < 0)
			FTS_ERROR("set TP to exit charger mode, ret=%d", ret);
	}
	return;
}
EXPORT_SYMBOL(fts_is_charger_enabled);

#if defined(CONFIG_PM) && FTS_PATCH_COMERR_PM
static void fts_gesture_func(struct work_struct *work)
{
	struct fts_ts_data *ts_data = container_of(work,
		struct fts_ts_data, gesture_work.work);
	int ret = 0;

	pr_info("fts_gesture_func enter\n");
	if (!ts_data->gesture_mode) {
		pr_info("fts: gesture is disabled\n");
		return;
	}
	...
	if (0 == fts_gesture_readdata(ts_data, NULL)) {
		pr_info("fts: succuss to get gesture data in irq handler\n");
		return;
	} else if (!ret){
		pr_info("fts: read gesture data error, try again\n");
		queue_delayed_work(ts_data->ts_workqueue, &ts_data->gesture_work,
			msecs_to_jiffies(300));
	}
    //schedule_delayed_work(&ts_data->ts_workqueue, msecs_to_jiffies(10));
}
#endif
/*****************************************************************************
...
void fts_release_all_finger(void)
{
    ...
    max_touches = zui_num_of_fingers;
    FTS_FUNC_ENTER();
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
		  ...
		  if (p) {
            ...
            input_report_abs(data->input_dev, ABS_MT_TOUCH_MINOR, events[i].area);
...
#ifdef CONFIG_TP_CALLBACK
/* AW8680x TP event notify start */
		finger = events[i].id;
		data->tp_event[i].fid= finger;
		data->tp_event[i].type= 'D';
		data->tp_event[i].x = events[i].x;
		data->tp_event[i].y = events[i].y;
		if (i == data->touch_point -1)
			touch_event_call_notifier(data->touch_point, data->tp_event);
/* AW8680x Tp event notify end */
#endif
    ...
    zui_finger_touch = 1;
    if (va_reported && (zui_atr_key_touch == 0)) {
...
#ifdef CONFIG_TP_CALLBACK
/* AW8680x TP event notify start */
		/* data->tp_event[i].type= 'U';
		touch_event_call_notifier(0, data->tp_event); */
/* AW8680x Tp event notify end */
#endif
        } else {
...
static int fts_read_touchdata(struct fts_ts_data *data)
{
    ...
    ret = fts_read_reg(FTS_REG_GESTURE_OUTPUT_ADDRESS, &fingerprint);
    if (fingerprint == 0x27) {
        fts_fp_match = 0;
    } else if (fingerprint == 0x26) {
        fts_fp_match = 1;
    }
...
static int fts_read_parse_touchdata(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < max_touch_num; i++) {
        ...
        dev_dbg(data->dev, "data->touch_point = %d, events[i].flag = %d\n", data->touch_point, events[i].flag);
        dev_dbg(data->dev, "events[i].p = %d, events[i].area = %d\n", events[i].p, events[i].area);
        dev_dbg(data->dev, "events[i].x = %d, data.point_num = %d, i = %d\n", events[i].x, data->point_num, i);
...
static int fts_input_init(struct fts_ts_data *ts_data)
{
    ...
    input_mt_init_slots(input_dev, zui_num_of_fingers, INPUT_MT_DIRECT);
#else
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    zui_num_of_fingers = pdata->max_touch_number;
    zui_num_of_fingers = zui_num_of_fingers + ZUI_NUM_OF_SLOT;
    pr_info("syna zui_num_of_fingers %d\n", zui_num_of_fingers);
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    ts_data->charger_mode = fts_charger_mode;
    ...
    INIT_DELAYED_WORK(&ts_data->gesture_work, fts_gesture_func);
#endif
