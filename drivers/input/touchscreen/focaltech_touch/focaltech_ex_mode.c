...
int fts_get_orientation_mode(void)
{
    u8 val = 0;
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev = ts_data->input_dev;
    ...
    mutex_lock(&input_dev->mutex);
    fts_read_reg(FTS_REG_ORIENTATION_MODE_EN, &val);
    mutex_unlock(&input_dev->mutex);

    return val;
}
EXPORT_SYMBOL(fts_get_orientation_mode);

void fts_set_orientation_mode(int mode)
{
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev = ts_data->input_dev;
    ...
    switch (mode) {
        ...
        case 1:
        case 2:
            fts_write_reg(FTS_REG_ORIENTATION_MODE_EN, mode);
            break;
        default:
            break;
    }
    ...
    pr_debug("Orientation mode:%d", mode);
    return;
}
EXPORT_SYMBOL(fts_set_orientation_mode);
