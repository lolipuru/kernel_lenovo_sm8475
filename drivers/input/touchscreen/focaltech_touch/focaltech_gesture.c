...
void fts_gesture_enable(int enable)
{
    struct fts_ts_data *ts_data = fts_data;
...
EXPORT_SYMBOL(fts_gesture_enable);
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    /* report event key */
    if (gesture != -1) {
        pr_info("[FTS]Gesture Code=%d", gesture);
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_ENTER();
    input_set_capability(input_dev, EV_KEY, KEY_TP_DOU_TAP);

    __set_bit(KEY_TP_DOU_TAP, input_dev->keybit);
